...
function StatusBadge({ status }) {
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${STATUS_STYLES[status] ?? STATUS_STYLES.pending}`}
    >
      {status}
    </span>
  );
...
function resolveHostName(host) {
  ...
  if (p)
    return (
      [p.firstName, p.lastName].filter(Boolean).join(" ") || host.email || "—"
    );
  return host.email || "—";
}
...
/* ── Reason cell with hover preview ───────────────────────────────────────── */
function ReasonCell({ reason }) {
  const [open, setOpen] = useState(false);

  if (!reason) return <p className="text-sm 2xl:text-base text-gray-600">—</p>;
  ...
  return (
    ...
    >
      <p className="text-sm 2xl:text-base text-gray-600 truncate cursor-default">
        {reason}
      </p>
      {open && (
        <div className="absolute z-20 left-0 top-full mt-1 w-64 max-w-xs p-3 bg-gray-900 text-white text-xs rounded-lg shadow-lg whitespace-pre-wrap wrap-break-word">
          {reason}
        </div>
      )}
    </div>
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl">
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          <p className="text-sm text-gray-500">
            The guest will be notified. Funds will still release to the host on
            day 7.
          </p>
          <div>
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 border border-gray-200 text-gray-500 font-semibold rounded-full text-sm hover:bg-gray-50 transition-colors"
            >
              Cancel
            </button>
            <button
...
export default function AdminRefundRequestsPage() {
  ...
  const handleReject = async (note) => {
    ...
    try {
      await dispatch(
        rejectRefundRequest({ requestId: rejectTarget, note }),
      ).unwrap();
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            {activeTab === "pending" && pagination.total > 0 && (
              <p className="text-sm 2xl:text-base text-yellow-600 font-medium mt-0.5">
                ...
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-5 2xl:px-8 py-3 2xl:py-5 text-left text-xs 2xl:text-sm font-semibold text-gray-500 uppercase tracking-wide whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {requests.map((req) => {
                    ...
                    const guestEmail =
                      typeof req.requestedBy === "object"
                        ...
                        : null;
                    const guestAvatar =
                      req.requestedBy?.providerProfile?.profileImage || null;
                    const hostName = resolveHostName(req.host);
                    const hostEmail =
                      typeof req.host === "object" ? req.host?.email : null;
                    const bookingRef = resolveBookingRef(req.booking);
                    const bookingTotal =
                      typeof req.booking === "object"
                        ? req.booking?.totalAmount
                        : null;
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-2.5">
                            {guestAvatar ? (
                              <img
                                src={guestAvatar}
                                alt=""
                                className="w-8 h-8 2xl:w-11 2xl:h-11 rounded-full object-cover shrink-0 border border-gray-100"
                              />
                            ) : (
                            ...
                            <div className="min-w-0">
                              <p className="text-sm 2xl:text-base font-semibold text-gray-800 truncate max-w-30 2xl:max-w-48">
                                {guestName}
                              </p>
                              {guestEmail && (
                                <p className="text-xs 2xl:text-sm text-gray-400 truncate max-w-30 2xl:max-w-48">
                                  {guestEmail}
                                </p>
                              )}
                            </div>
                        ...
                        {/* Host */}
                        <td className="px-5 2xl:px-8 py-3.5 2xl:py-5">
                          <p className="text-sm 2xl:text-base font-semibold text-gray-800 truncate max-w-32.5">
                            {hostName}
                          </p>
                          {hostEmail && (
                            <p className="text-xs 2xl:text-sm text-gray-400 truncate max-w-32.5">
                              {hostEmail}
                            </p>
                          )}
                        </td>
                        ...
                        {/* Booking Ref */}
                        <td className="px-5 2xl:px-8 py-3.5 2xl:py-5">
                          <p className="text-sm 2xl:text-base font-mono font-semibold text-gray-700">
                            #{bookingRef}
                          </p>
                          {bookingTotal != null && (
                        ...
                        <td className="px-5 2xl:px-8 py-3.5 2xl:py-5 whitespace-nowrap">
                          <p className="text-sm 2xl:text-base font-bold text-gray-900">
                            {req.amount != null
                              ...
                              : "—"}
                            {req.currency && (
                              <span className="text-xs 2xl:text-sm font-normal text-gray-400 ml-1">
                                {req.currency}
                              </span>
                            )}
                          </p>
