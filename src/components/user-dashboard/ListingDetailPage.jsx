...
function ImageGallery({ images, title }) {
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      <div className="relative w-full overflow-hidden rounded-[22px] bg-gray-100 flex-1 min-h-0" style={{ minHeight: 320 }}>
...
function TimeSelect({ value, onChange, placeholder = "Select time" }) {
  return (
    <div className="relative">
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className="w-full appearance-none bg-transparent text-sm font-medium text-gray-900 focus:outline-none pr-6"
      >
      ...
      <svg className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
...
function BookingField({ icon, children }) {
  return (
    <div className="flex items-center gap-2 rounded-full border border-gray-200 bg-white px-4 py-3 text-sm text-gray-500 focus-within:border-[#4AA7A7]">
      {icon}
...
function BookingShell({ children, title, price, priceUnit, rating, reviews }) {
  ...
  return (
    <div className="w-full rounded-[24px] border border-gray-200 bg-white p-5 flex flex-col gap-4 h-full">
      {children}
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right shrink-0">
          ...
          <span className="text-base text-gray-400">{priceUnit}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <BookingShell title="Book Your Activity" price={listing.price} priceUnit="/Hr" rating={listing.rating} reviews={listing.reviews}>
      <div className="space-y-3">
        <div className="rounded-[18px] border border-[#4AA7A7] bg-[#D7ECEB] px-4 py-4 text-center relative">
          <CheckBadge />
          <div className="flex flex-col items-center gap-1">
        ...
        <div className="grid grid-cols-2 gap-3">
          <BookingField icon={<ClockIcon />}>
            <div>
              <p className="text-[10px] text-gray-400 leading-none mb-1">Start time</p>
              <TimeSelect value={startTime} onChange={setStartTime} placeholder="09:41 AM" />
            </div>
          </BookingField>
          <BookingField icon={<ClockIcon />}>
            <div>
              <p className="text-[10px] text-gray-400 leading-none mb-1">End time</p>
              <TimeSelect value={endTime} onChange={setEndTime} placeholder="09:41 AM" />
            </div>
          </BookingField>
        </div>
        ...
        <div className="space-y-1.5 border-t border-gray-100 pt-3">
          <SummaryLine label={`$${listing.price}/hr x ${hours} hour${hours > 1 ? "s" : ""}`} value={`$${subtotal}`} />
        ...
        <button
          ...
          className="w-full rounded-full bg-[#F5C842] hover:bg-[#e0b430] py-3.5 text-sm font-semibold text-gray-900 transition-colors"
        >
          Reserve
        </button>
      </div>
      ...
      <div className="space-y-3">
        <div className="rounded-[18px] border border-[#4AA7A7] bg-[#D7ECEB] px-4 py-4 text-center relative">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-sm font-medium text-gray-700">Per Hour</p>
          </div>
        </div>
        ...
        <div className="space-y-1.5 border-t border-gray-100 pt-3">
          <SummaryLine label={`$${listing.price}/hr x ${hours} hours`} value={`$${subtotal}`} />
          <SummaryLine label="Service fee" value={`$${fee}`} />
          <SummaryLine label="Total" value={`$${total}`} bold />
        </div>
  ...
  return (
    <BookingShell title="Book Your Equipment" price={listing.price} priceUnit={mode === "daily" ? "/Day" : "/Hr"} rating={listing.rating} reviews={listing.reviews}>
      <div className="space-y-3">
        ...
        <div className="space-y-1.5 border-t border-gray-100 pt-3">
          <SummaryLine label={`$${listing.price}/${mode === "daily" ? "day" : "hr"} x ${span} ${mode === "daily" ? "day" : "hour"}${span > 1 ? "s" : ""}`} value={`$${subtotal}`} />
          <SummaryLine label="Service fee" value={`$${fee}`} />
          <SummaryLine label="Total" value={`$${total}`} bold />
        </div>
...
/* ─── Location Map with pin + popup card ─────────────────────────────────────── */
function ListingLocationMap({ listing }) {
  const [coords, setCoords] = useState(null);
  const [showCard, setShowCard] = useState(true);

  useEffect(() => {
    // Use API coordinates if present and not the old hardcoded fallback
    const hasRealCoords =
      listing.mapLat != null &&
      listing.mapLng != null &&
...
function DetailsSection({ listing }) {
  ...
  return (
    <div className="rounded-[22px] border border-gray-200 bg-white p-5">
      ...
      {/* Top row: title, location, category */}
...
export default function ListingDetailPage({ params: paramsPromise }) {
  ...
  return (
    <div className="flex flex-col bg-gray-50 min-h-screen">
      <main className="flex-1 max-w-350 mx-auto w-full px-4 sm:px-5 lg:px-8 py-5 sm:py-6 lg:py-8">

        {/* Header row */}
        <div className="mb-5 flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <button onClick={() => router.back()} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-700 hover:border-[#4AA7A7] hover:text-[#4AA7A7] transition-colors">
              <BackIcon />
            </button>
            <h1 className="truncate text-xl font-semibold text-gray-900">{listing.title}</h1>
          </div>
          <div className="flex shrink-0 items-center gap-2">
            <button className="flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 hover:border-[#4AA7A7] hover:text-[#4AA7A7] transition-colors"><HeartIcon /></button>
            <button className="flex h-10 w-10 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 hover:border-[#4AA7A7] hover:text-[#4AA7A7] transition-colors"><ShareIcon /></button>
          </div>
        </div>

        {/* Image gallery + Booking card — equal height */}
        <div className="grid gap-5 lg:grid-cols-2 lg:gap-6 items-stretch">
