...
export default function PreferenceSelectionPage() {
  ...
  const handleLater = () => router.push("/user-dashboard/explore");
        ...
        <div className="bg-white rounded-[2rem] w-full max-w-4xl flex flex-col max-h-[90vh] overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <section className="mb-10">
              <h2 className="text-sm font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-6">Amenities</h2>
              <div className="flex flex-wrap gap-3">
                {CATEGORIES.AMENITIES.map(item => (
                  <Tag key={item} label={item} isSelected={selected.amenities.includes(item)} onClick={() => toggleSelection('amenities', item)} />
                ))}
                <button className="text-sm font-bold text-[var(--color-secondary)] hover:underline ml-2">See 24+ more</button>
              </div>
            </section>

            <section className="mb-10">
              <h2 className="text-sm font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-6">Equipment&apos;s</h2>
              <div className="flex flex-wrap gap-3">
                {CATEGORIES.EQUIPMENT.map(item => (
                  <Tag key={item} label={item} isSelected={selected.equipment.includes(item)} onClick={() => toggleSelection('equipment', item)} />
                ))}
                <button className="text-sm font-bold text-[var(--color-secondary)] hover:underline ml-2">See 24+ more</button>
              </div>
            </section>
            ...
            <section className="mb-4">
              <h2 className="text-sm font-bold text-[var(--color-text-muted)] uppercase tracking-wider mb-6">Services</h2>
              <div className="flex flex-wrap gap-3">
                {CATEGORIES.SERVICES.map(item => (
                  <Tag key={item} label={item} isSelected={selected.services.includes(item)} onClick={() => toggleSelection('services', item)} />
                ))}
              </div>
            </section>
          </div>
