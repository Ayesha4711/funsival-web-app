...
function ContactPanel({ activeContact, onSelect, searchQuery, setSearchQuery, activeTab, setActiveTab }) {
  ...
  const filtered = CONTACTS.filter(c => {
    ...
    if (activeTab === "Favourites") return false; // no favourites in mock
      ...
      <div className="flex-1 overflow-y-auto px-2">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-2">
            ...
            <p className="text-sm font-medium text-[var(--color-text-muted)] bg-orange-50 border border-orange-100 text-orange-400 px-4 py-1.5 rounded-full">
              No result found
            </p>
          </div>
        ) : (
...
function ChatWindow({ contact, onBack, showBackBtn }) {
  ...
  const sendMessage = () => {
    ...
    if (!trimmed) return;
    setMessages(prev => [...prev, { id: Date.now(), from: "me", text: trimmed, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }]);
    setText("");
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-4 sm:px-6 py-4 border-b border-gray-100 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-green-500 font-medium">{contact.online ? "Online" : "Offline"}</p>
        </div>
        <ChatMenu />
      </div>
      ...
      <div className="px-4 sm:px-6 py-4 border-t border-gray-100 shrink-0">
        <div className="flex items-center gap-3 bg-gray-50 rounded-full px-4 py-2">
          <button className="text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors shrink-0">
            <EmojiIcon />
          </button>
          <button className="text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors shrink-0">
            <AttachIcon />
...
/* ─── Empty state (no contact selected on desktop) ───────────────────────────── */
...
export default function MessagesPage() {
  ...
  // On mobile/tablet: null = show list, contact = show chat
  const [mobileView, setMobileView] = useState(null);

  const handleSelectContact = (contact) => {
    setActiveContact(contact);
    setMobileView(contact); // switch to chat on mobile
  };
    ...
    <div className="flex flex-col" style={{ height: "calc(100dvh - 64px)" }}>
      ...
      <div className="flex-1 min-h-0 px-4 sm:px-6 lg:px-8 pb-4 sm:pb-6">
        <div className="h-full bg-white rounded-3xl border border-[var(--color-border)] shadow-sm overflow-hidden flex">
          ...
          {/* ── Contact list panel ───────────────────────────────────── */}
          {/* Desktop: always visible. Mobile/tablet: only when no chat open */}
          <div className={`
          ...
          {/* ── Chat window ──────────────────────────────────────────── */}
          {/* Desktop: show selected or empty state. Mobile: only when contact chosen */}
          <div className={`
            flex-1 min-w-0
