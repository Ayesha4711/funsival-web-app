...
const footerLinks = {
  Support: ["Help Center", "Cancellation Policy", "Privacy Policy", "Terms & Conditions", "FAQ's", "Report"],
  Hosting: ["Funsival your home", "Hosting resources", "Hosting Responsibility", "Community Forum"],
  ...
  return (
    <footer className="bg-white border-t border-[var(--color-border)]">
      ...
      <div className="px-6 sm:px-10 lg:px-16 py-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-b border-[var(--color-border)]">
        <div>
          <h3 className="text-xl font-bold text-[var(--color-text)] mb-1">Keep In Touch Through Our Newsletter!</h3>
          <p className="text-sm text-[var(--color-text-muted)]">Lorem ipsum dolor sit amet consectetur.</p>
        </div>
      ...
      <div className="px-6 sm:px-10 lg:px-16 py-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        {Object.entries(footerLinks).map(([heading, links]) => (
          <div key={heading}>
            <h4 className="text-sm font-bold text-[var(--color-text)] mb-4">{heading}</h4>
      ...
      <div className="bg-[var(--color-primary)] px-6 sm:px-10 lg:px-16 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
        <p className="text-white/80 text-sm">Copyright © 2023 Funsival| All Rights Reserved</p>
        <div className="flex items-center gap-4">
          <button className="text-white/80 hover:text-white transition-colors"><InstagramIcon /></button>
          <button className="text-white/80 hover:text-white transition-colors"><XIcon /></button>
          <button className="text-white/80 hover:text-white transition-colors"><YoutubeIcon /></button>
        </div>
      </div>
    </footer>
  );
